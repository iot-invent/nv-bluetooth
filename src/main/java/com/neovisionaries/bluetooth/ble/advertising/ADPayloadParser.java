...
/**
 * Utility to parse a payload of an advertising packet. Especially, the third argument of <a href=
 * "http://developer.android.com/reference/android/bluetooth/BluetoothAdapter.LeScanCallback.html#onLeScan%28android.bluetooth.BluetoothDevice,%20int,%20byte[]%29"
 * >onLeScan</a> method of android.<wbr>bluetooth.<wbr>BluetoothAdapter.<wbr>LeScanCallback interface.
 *
...
public class ADPayloadParser {
	...
	protected ADPayloadParser() {
		...
		final UUIDsBuilder uuidsBuilder = new UUIDsBuilder();

		// Builder for AD structures that contain a local name.
		final LocalNameBuilder localNameBuilder = new LocalNameBuilder();

		// Builder for AD structures for Service Data.
		final ServiceDataBuilder serviceDataBuilder = new ServiceDataBuilder();
	...
	/**
	 * Register an AD structure builder for the AD type. The given builder is added at the beginning of the list of the
	 * builders for the AD type.
	 *
	 * <p>
	 * Note that a builder for the type <i>Manufacturer Specific Data</i> (0xFF) should not be registered by this
	 * method. Instead, use {@link #registerManufacturerSpecificBuilder(int, ADManufacturerSpecificBuilder)}.
	 * </p>
	 *
	 * @param type
	 ...
	 */
	public void registerBuilder(final int type, final ADStructureBuilder builder) {
		if (type < 0 || 0xFF < type) {
			final String message = String.format("'type' is out of the valid range: %d", type);
	...
	/**
	 * Register a builder for the company ID. The given builder is added at the beginning of the list of the builders
	 * for the company ID.
	 *
	 * @param companyId
	 ...
	 *            A builder.
	 */
	public void registerManufacturerSpecificBuilder(final int companyId, final ADManufacturerSpecificBuilder builder) {
		if (companyId < 0 || 0xFFFF < companyId) {
			final String message = String.format("'companyId' is out of the valid range: %d", companyId);
			throw new IllegalArgumentException(message);
		}
		...
		final Integer key = Integer.valueOf(companyId);
	 ...
	 *
	 * @return A list of parsed AD structures. If {@code payload} is {@code null}, this method returns {@code null}.
	 */
	public List<ADStructure> parse(final byte[] payload) {
		if (payload == null) {
			return null;
		}
	 ...
	 * registerManufacturerSpecificBuilder} can be used to register your customized parsers for AD structures.
	 * </p>
	 *
	 * <p>
	 * {@link ServiceData} has subclasses. The table below lists the supported data formats.
	 * </p>
	 *
	 * <blockquote>
	 * <table border="1" cellpadding="5" style="border-collapse: collapse;">
	 ...
	 * @return A list of parsed AD structures. If {@code payload} is {@code null}, this method returns {@code null}.
	 */
	public List<ADStructure> parse(final byte[] payload, final int offset, final int length) {
		if (payload == null) {
			return null;
		}

		// List of AD structures.
		final List<ADStructure> list = new ArrayList<ADStructure>();
		...
		final int end = Math.min(offset + length, payload.length);

		for (int i = offset; i < end;) {
			// The first octet represents 'length'.
			final int len = payload[i] & 0xFF;
			...
			// The AD data.
			final byte[] data = Arrays.copyOfRange(payload, i + 2, i + len + 1);

			// Build an AD structure.
			final ADStructure ads = buildAds(len, type, data);
	...
	private ADStructure buildAds(final int length, final int type, final byte[] data) {
		// Get the list of builders for the AD type.
		final List<ADStructureBuilder> builders = mBuilders.get(Integer.valueOf(type));
		...
		for (final ADStructureBuilder builder : builders) {
			// Let the builder build an AD structure.
			final ADStructure structure = builder.build(length, type, data);
	...
	private class MSBuilder implements ADStructureBuilder {
		public ADStructure build(final int length, final int type, final byte[] data) {
			...
			final int companyId = ((data[1] & 0xFF) << 8) | (data[0] & 0xFF);

			// Get the list of builders for the company ID.
			final List<ADManufacturerSpecificBuilder> builders = mMSBuilders.get(Integer.valueOf(companyId));
